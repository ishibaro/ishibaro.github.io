...
function doDelUE(id){
  ...
  if(!confirm(delUeQ(u.nombre,rels))){refocusWin();return;}
...
function doDelSec(id){
  ...
  if(!confirm(delSecQ(s.nombre||'—',ues))){refocusWin();return;}
...
function tryClose(){if(formDirty&&!confirm(t('discard_q'))){refocusWin();return;}closeModal();}
