...
async function startNewProjectFlow(){
  ...
  if(!created)return; // user cancelled the discard-changes prompt
  projectFlow=true;openProject();
}
...
async function newProject(){
  if(DB.unidades.length||DB.relaciones.length){
    const ok=await customConfirm(t('new_proj_q'));
    if(!ok)return false;
  }
...
if(window.electronAPI){
  window.electronAPI.onMenuAction(action=>{
    if(action==='new'){startNewProjectFlow();}
